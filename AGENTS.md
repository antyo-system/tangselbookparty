...
```
/src
  /assets        # Optimized brand logo (tbp-logo.png) & graphics
  /components    # Reusable UI components (Navbar, Footer, Modals, LoginModal)
  /pages         # Main page views (CatalogPage, EventsPage, ArticlesPage, ProfilePage, AdminDashboard)
  /services      # Data & backend persistence (storage.ts, supabase.ts)
  /types         # Shared TypeScript interfaces (Book, BorrowRequest, Member, CommunityEvent, Article)
  App.tsx        # React root & tab routing
  index.css      # Design system tokens & global utility classes
supabase_schema.sql  # Database tables & initial seed SQL script
```
