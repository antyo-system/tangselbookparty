...
export const BottomNav: React.FC<BottomNavProps> = ({
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 w-full max-w-full z-40 bg-white/95 backdrop-blur-lg border-t border-slate-200 shadow-xl md:hidden overflow-hidden box-border">
      <div className="flex items-center justify-between h-16 w-full max-w-full px-1 box-border">
        ...
        >
          <div className={`p-1.5 rounded-xl transition-all flex flex-col items-center justify-center ${activeTab === 'catalog' ? 'bg-emerald-100/80 text-[#053D27]' : ''}`}>
        ...
        >
          <div className={`p-1.5 rounded-xl transition-all flex flex-col items-center justify-center ${activeTab === 'events' ? 'bg-emerald-100/80 text-[#053D27]' : ''}`}>
            <Calendar className="w-5 h-5" />
          </div>
          <span className="text-[10px] mt-0.5 font-extrabold tracking-tight truncate max-w-full">Acara</span>
        </button>

        {/* 3. Quick QR Scanner (PERFECT CENTER 50% FLOATING BUTTON) */}
        <button
          type="button"
          onClick={onOpenScanner}
          className="flex flex-col items-center justify-center -mt-4 mx-0.5 flex-shrink-0 outline-none focus:outline-none focus:ring-0 select-none"
        ...
        {/* 4. Artikel (Right 1 - SEO Articles & Guides) */}
        <button
          type="button"
        ...
        >
          <div className={`p-1.5 rounded-xl transition-all flex flex-col items-center justify-center ${activeTab === 'articles' ? 'bg-emerald-100/80 text-[#053D27]' : ''}`}>
        ...
        >
          <div className={`p-1.5 rounded-xl transition-all flex flex-col items-center justify-center relative ${activeTab === 'profile' ? 'bg-emerald-100/80 text-[#053D27]' : ''}`}>
